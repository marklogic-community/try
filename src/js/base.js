...
(function () {
  ...
  if(typeof window.tryml === 'undefined' || !window.tryml) {
    window.tryml = tryml;
  }
  ...
  tryml.blockToParserConfig = function(block, type) {
    ...
    else if(block.hasClass(type + '-sjs')) {
      config.mode = {'name': 'javascript', 'json': true};
    }
    ...
    if (window.console) {
      window.console.log(type + ' mode: ' + config.mode);
    }
  ...
  //      }
        ...
        $.ajax({
          url: tryml.urlPrefix + '/exec.xqy',
          data: {
            code: inputEditor.getValue(),
            format: inputEditor.getDoc().getMode().name
          },
          dataType: 'jsonp',
